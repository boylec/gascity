...
func doOrderRun(aa []orders.Order, name, rig, cityPath string, store beads.Store, ep events.Provider, stdout, stderr io.Writer) int {
	...
	if a.Pool != "" && cfg != nil {
		pool := resolvePoolTemplate(qualifyPool(a.Pool, a.Rig), cfg)
	...
	if a.Pool != "" {
		update.Metadata = map[string]string{
			"gc.routed_to": resolvePoolTemplate(qualifyPool(a.Pool, a.Rig), cfg),
		}
	}
	...
	if a.Pool != "" {
		fmt.Fprintf(stdout, " → gc.routed_to=%s", resolvePoolTemplate(qualifyPool(a.Pool, a.Rig), cfg)) //nolint:errcheck
	}
